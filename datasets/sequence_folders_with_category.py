...
def load_seg_and_category_as_tensor(path):
    npz = np.load(path)
    return torch.from_numpy(npz["masks"].astype(np.float32)), torch.from_numpy(npz["classes"].astype(np.uint8))
...
class SequenceFolder(data.Dataset):
    ...
    def __init__(
        ...
        self.height_priors, self.height_var_priors = SequenceFolder.read_height_priors(root_dir)

    @staticmethod
    def read_height_priors(root_dir: Path):
        height_priors_path = root_dir / "height_priors.npy"
        height_var_priors_path = root_dir / "height_var_priors"
