...
class SequenceFolder(data.Dataset):
    ...
    def crawl_folders(self, sequence_length):
        ...
        for scene in self.scenes:
            sceneff = Path(scene).parents[1] / "flow_f" / scene.split("/")[-1]
            scenefb = Path(scene).parents[1] / "flow_b" / scene.split("/")[-1]
            scenei = Path(scene).parents[1] / "segmentation" / scene.split("/")[-1]
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(ref_imgs)):
            ...
            if seg0_re[0].mean() != 0 and seg0_re[int(seg0_re[0].mean())].mean() == 0:
                pdb.set_trace()
            if seg1_re[0].mean() != 0 and seg1_re[int(seg1_re[0].mean())].mean() == 0:
                pdb.set_trace()
