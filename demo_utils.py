...
from rigid_warp import depth2flow, flow_warp, inverse_warp2, pixel2cam, transform_scale_consistent_depth
...
def compute_obj_translation(r2t_obj_depths, t2r_obj_depths, tgt_obj_depths, ref_obj_depths, num_insts, intrinsics):
    ...
    for r2t_obj_depth, t2r_obj_depth, tgt_obj_depth, ref_obj_depth, num_inst in zip(
...
def compute_batch_bg_warping(
    tgt_img, ref_imgs, tgt_bg_masks, ref_bg_masks, tgt_depth, ref_depths, poses, poses_inv, intrinsics
):
    outputs = []
    for ref_img, ref_depth, pose, pose_inv, tgt_bg_mask, ref_bg_mask in zip(
        ref_imgs, ref_depths, poses, poses_inv, tgt_bg_masks, ref_bg_masks
    ):
        """
        tgt_img:     ([B, 3, 256, 832])
        ref_img:     ([B, 3, 256, 832])
        ...
        fwd_outputs = compute_bg_warping(ref_img, tgt_bg_mask, tgt_depth, ref_depth, pose, intrinsics)
        bwd_outputs = compute_bg_warping(tgt_img, ref_bg_mask, ref_depth, tgt_depth, pose_inv, intrinsics)

        outputs.append([torch.cat([fwd, bwd], dim=0) for fwd, bwd in zip(fwd_outputs, bwd_outputs)])
    return outputs


def compute_bg_warping(ref_img, bg_mask, tgt_depth, ref_depth, pose, intrinsic):
    ref_img_warped, valid_mask, projected_depth, computed_depth = inverse_warp2(
        ref_img, tgt_depth, pose, intrinsic, ref_depth
    )
...
def compute_batch_obj_warping(
    ...
    for ref_img, ref_depth, tgt_obj_mask, ref_obj_mask, ego_pose, ego_pose_inv, obj_pose, obj_pose_inv, num_inst in zip(
        ref_imgs,
        ref_depths,
        tgt_obj_masks,
        ref_obj_masks,
        ...
        fwd_outputs, fwd_ovl_obj = compute_obj_warping(
            ref_Is, ref_obj_mask, tgt_obj_mask, ref_Ds, tgt_Ds, ego_Ps, obj_pose, Ks, num_inst
        )

        bwd_outputs, bwd_ovl_obj = compute_obj_warping(
            tgt_Is, tgt_obj_mask, ref_obj_mask, tgt_Ds, ref_Ds, ego_Ps_inv, obj_pose_inv, Ks, num_inst
        )
...
def compute_obj_warping(
    ref_img, ref_obj_mask, tgt_obj_mask, ref_depth, tgt_depth, ego_pose, obj_pose, intrinsic, num_inst
):
    rtt_img_warped, valid_mask, projected_depth, computed_depth = inverse_warp2(
        ref_img, tgt_depth, [ego_pose, obj_pose], intrinsic, ref_depth
    )
    ...
    for bb, ni in enumerate(num_inst):
        if ni == 0:
            ...
            prj_Ds.append(
                torch.zeros(1, projected_depth.size(1), projected_depth.size(2), projected_depth.size(3)).cuda()
            )
            ...
            continue
        rtt_Is.append(
            (rtt_img_warped * obj_valid_mask)[int(sum(num_inst[:bb])) : int(sum(num_inst[:bb]) + ni)].sum(
                dim=0, keepdim=True
            )
        )
        rtt_Ms.append(obj_valid_mask[int(sum(num_inst[:bb])) : int(sum(num_inst[:bb]) + ni)].sum(dim=0, keepdim=True))
        prj_Ds.append(
            (projected_depth * obj_valid_mask)[int(sum(num_inst[:bb])) : int(sum(num_inst[:bb]) + ni)].sum(
                dim=0, keepdim=True
            )
        )
        cmp_Ds.append(
            (computed_depth * obj_valid_mask)[int(sum(num_inst[:bb])) : int(sum(num_inst[:bb]) + ni)].sum(
                dim=0, keepdim=True
            )
        )
        ...
        ovl_Ms.append(
            (1 - (1 - tgt_obj_mask) * (1 - ref_obj_mask))[int(sum(num_inst[:bb])) : int(sum(num_inst[:bb]) + ni)]
