...
def compute_batch_obj_warping(
    tgt_img, ref_imgs, tgt_obj_masks, ref_obj_masks, tgt_depth, ref_depths, ego_poses, ego_poses_inv, obj_poses, obj_poses_inv, intrinsics, num_insts
):
    outputs, ovl_obj = [], []

    for ref_img, ref_depth, tgt_obj_mask, ref_obj_mask, ego_pose, ego_pose_inv, obj_pose, obj_pose_inv, num_inst in zip(
        ref_imgs, ref_depths, tgt_obj_masks, ref_obj_masks, ego_poses, ego_poses_inv, obj_poses, obj_poses_inv, num_insts
    ):
        # pdb.set_trace()
...
def compute_obj_warping(ref_img, ref_obj_mask, tgt_obj_mask, ref_depth, tgt_depth, ego_pose, obj_pose, intrinsic, num_inst):
    ...
    for bb, ni in enumerate(num_inst):
        ...
        ovl_Ms.append(
            (1 - (1 - tgt_obj_mask) * (1 - ref_obj_mask))[int(sum(num_inst[:bb])) : int(sum(num_inst[:bb]) + ni)].sum(dim=0, keepdim=True).clamp(0, 1)
        )  # ref + tgt 모두 합해서 마스킹
