...
parser.add_argument(
    "--pretrained-disp", dest="pretrained_disp", default=None, metavar="PATH", help="path to pre-trained DispResNet"
)
parser.add_argument(
    "--pretrained-ego-pose",
    ...
    default=None,
    metavar="PATH",
    help="path to pre-trained EgoPoseNet",
)
parser.add_argument(
    "--pretrained-obj-pose",
    ...
    default=None,
    metavar="PATH",
    help="path to pre-trained ObjPoseNet",
)
...
parser.add_argument(
    "-hp", "--height-loss-weight", type=float, help="weight for height prior loss", metavar="W", default=0.0
)
parser.add_argument(
    "-dm", "--depth-loss-weight", type=float, help="weight for depth mean loss", metavar="W", default=0.0
)
...
def main():
    ...
    if args.with_gt:
        ...
        valid_transform = custom_transforms.ComposeWithEgoPose(
            [custom_transforms.ArrayToTensorWithEgoPose(), normalize]
        )
        ...
        if args.pretrained_disp:
            ...
            if args.with_gt:
                ...
                errors, error_names = validate_without_gt(
                    args, val_loader, disp_net, ego_pose_net, obj_pose_net, 0, logger
                )
    ...
    for epoch in range(args.epochs):
        ...
        if args.with_gt:
            ...
            errors, error_names = validate_without_gt(
                args, val_loader, disp_net, ego_pose_net, obj_pose_net, epoch, logger
            )
        error_string = ", ".join("{} : {:.3f}".format(name, error) for name, error in zip(error_names, errors))
        logger.valid_writer.write(" * Avg {}".format(error_string))
