...
parser.add_argument(
    "-j", "--workers", default=os.cpu_count() - 2, type=int, metavar="N", help="number of data loading workers"
)
...
def main():
    ...
    # TODO: change for without pose GT
    ...
    disp_net = models.DispResNet(args.resnet_layers, args.with_pretrained, args.num_categories).to(device)
...
def train(args, train_loader, disp_net, ego_pose_net, obj_pose_net, optimizer, epoch_size, logger, tf_writer):
    ...
    for i, (
        ...
        (ego_poses_fwd, ego_poses_bwd, translation_loss, rotation_loss, direction_loss,) = compute_ego_pose_with_inv(
        ...
        if w4 == 0:
            ...
            # TODO: おそらくこの中でカOテゴリーごとにheight priorを分ける
            # TODO: ガウス分布の中心に従うかのロスを設定する
            # TODO: validation lossをスケール調整なしに評価する
