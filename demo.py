...
def demo_visualize(args, demo_loader, disp_net, ego_pose_net, obj_pose_net):
    ...
    for i, (ref_img, tgt_img, ref_seg, tgt_seg, intrinsics, intrinsics_inv) in enumerate(demo_loader):
        ...
        IMDDs = compute_batch_bg_warping(
            tgt_img,
            [ref_img, ref_img],
            [tgt_bg_mask, tgt_bg_mask],
            [ref_bg_mask, ref_bg_mask],
            tgt_depth,
            [ref_depth, ref_depth],
            [ego_pose, ego_pose],
        ...
        plt.imshow(ref_masked, vmax=1)
        plt.text(text_xy[0], text_xy[1], "$I_{t}$", fontdict=text_fd)
        ...
        plt.imshow(d_ref, cmap="turbo", vmax=14)
        plt.text(text_xy[0], text_xy[1], "$D_{t}$", fontdict=text_fd)
        plt.xticks([]) and plt.yticks([]) if args.save_fig else plt.grid(linestyle=":", linewidth=0.4)
        plt.xlim(0, 832 - 1)
        plt.ylim(256 - 1, 0)
        fig.add_subplot(gs[1, 0:2])
        plt.imshow(tgt_masked, vmax=1)
        plt.text(text_xy[0], text_xy[1], "$I_{t+1}$", fontdict=text_fd)
        plt.xticks([]) and plt.yticks([]) if args.save_fig else plt.grid(linestyle=":", linewidth=0.4)
        plt.xlim(0, 832 - 1)
        plt.ylim(256 - 1, 0)
        fig.add_subplot(gs[1, 2:4])
        plt.imshow(d_tgt, cmap="turbo", vmax=14)
        plt.text(text_xy[0], text_xy[1], "$D_{t+1}$", fontdict=text_fd)
        plt.xticks([]) and plt.yticks([]) if args.save_fig else plt.grid(linestyle=":", linewidth=0.4)
        plt.xlim(0, 832 - 1)
        plt.ylim(256 - 1, 0)
        fig.add_subplot(gs[2, 0:2])
        plt.imshow(r2t_obj, vmax=1)
        plt.text(text_xy[0], text_xy[1], "Ego-warped objects with motion", fontdict=text_fd, size=10)
        plt.xticks([]) and plt.yticks([]) if args.save_fig else plt.grid(linestyle=":", linewidth=0.4)
        plt.text(
            130,
            250,
        ...
        if num_inst > 0 and not args.save_fig:
            plt.text(
                330,
                7,
        ...
        if num_inst > 1 and not args.save_fig:
            plt.text(
                330,
                31,
        ...
        if num_inst > 2 and not args.save_fig:
            plt.text(
                330,
                55,
        ...
        if num_inst > 3 and not args.save_fig:
            plt.text(
                330,
                79,
        ...
        if num_inst > 0 and not args.save_fig:
            plt.arrow(
                r2t_obj_tail[0, 0],
                r2t_obj_tail[0, 1],
                arr_scale * (-r2t_obj_tail[0, 0] + r2t_obj_head_gt[0, 0]),
                arr_scale * (-r2t_obj_tail[0, 1] + r2t_obj_head_gt[0, 1]),
                width=2,
                head_width=9,
                head_length=9,
                color="red",
        ...
        if num_inst > 0:
            plt.arrow(
                r2t_obj_tail[0, 0],
                r2t_obj_tail[0, 1],
                arr_scale * (-r2t_obj_tail[0, 0] + r2t_obj_head[0, 0]),
                arr_scale * (-r2t_obj_tail[0, 1] + r2t_obj_head[0, 1]),
                width=3,
                head_width=10,
                head_length=9,
                color="magenta",
        ...
        if num_inst > 0:
            plt.text(
                r2t_obj_tail[0, 0] - 30,
                r2t_obj_tail[0, 1] + 25,
                "1: {:0.4f}".format(float(obj_pose[0, :3].pow(2).sum().sqrt())),
                fontsize=7,
                bbox=bbox_l,
            )
        if num_inst > 1 and not args.save_fig:
            plt.arrow(
                r2t_obj_tail[1, 0],
                r2t_obj_tail[1, 1],
                arr_scale * (-r2t_obj_tail[1, 0] + r2t_obj_head_gt[1, 0]),
                arr_scale * (-r2t_obj_tail[1, 1] + r2t_obj_head_gt[1, 1]),
                width=2,
                head_width=9,
                head_length=9,
                color="red",
        ...
        if num_inst > 1:
            plt.arrow(
                r2t_obj_tail[1, 0],
                r2t_obj_tail[1, 1],
                arr_scale * (-r2t_obj_tail[1, 0] + r2t_obj_head[1, 0]),
                arr_scale * (-r2t_obj_tail[1, 1] + r2t_obj_head[1, 1]),
                width=3,
                head_width=10,
                head_length=9,
                color="magenta",
        ...
        if num_inst > 1:
            plt.text(
                r2t_obj_tail[1, 0] - 30,
                r2t_obj_tail[1, 1] + 25,
                "2: {:0.4f}".format(float(obj_pose[1, :3].pow(2).sum().sqrt())),
                fontsize=7,
                bbox=bbox_l,
            )
        if num_inst > 2 and not args.save_fig:
            plt.arrow(
                r2t_obj_tail[2, 0],
                r2t_obj_tail[2, 1],
                arr_scale * (-r2t_obj_tail[2, 0] + r2t_obj_head_gt[2, 0]),
                arr_scale * (-r2t_obj_tail[2, 1] + r2t_obj_head_gt[2, 1]),
                width=2,
                head_width=9,
                head_length=9,
                color="red",
        ...
        if num_inst > 2:
            plt.arrow(
                r2t_obj_tail[2, 0],
                r2t_obj_tail[2, 1],
                arr_scale * (-r2t_obj_tail[2, 0] + r2t_obj_head[2, 0]),
                arr_scale * (-r2t_obj_tail[2, 1] + r2t_obj_head[2, 1]),
                width=3,
                head_width=10,
                head_length=9,
                color="magenta",
        ...
        if num_inst > 2:
            plt.text(
                r2t_obj_tail[2, 0] - 30,
                r2t_obj_tail[2, 1] + 25,
                "3: {:0.4f}".format(float(obj_pose[2, :3].pow(2).sum().sqrt())),
                fontsize=7,
                bbox=bbox_l,
            )
        if num_inst > 3 and not args.save_fig:
            plt.arrow(
                r2t_obj_tail[3, 0],
                r2t_obj_tail[3, 1],
                arr_scale * (-r2t_obj_tail[3, 0] + r2t_obj_head_gt[3, 0]),
                arr_scale * (-r2t_obj_tail[3, 1] + r2t_obj_head_gt[3, 1]),
                width=2,
                head_width=9,
                head_length=9,
                color="red",
        ...
        if num_inst > 3:
            plt.arrow(
                r2t_obj_tail[3, 0],
                r2t_obj_tail[3, 1],
                arr_scale * (-r2t_obj_tail[3, 0] + r2t_obj_head[3, 0]),
                arr_scale * (-r2t_obj_tail[3, 1] + r2t_obj_head[3, 1]),
                width=3,
                head_width=10,
                head_length=9,
                color="magenta",
        ...
        if num_inst > 3:
            plt.text(
                r2t_obj_tail[3, 0] - 30,
                r2t_obj_tail[3, 1] + 25,
                "4: {:0.4f}".format(float(obj_pose[3, :3].pow(2).sum().sqrt())),
                fontsize=7,
                bbox=bbox_l,
            )
        plt.xlim(0, 832 - 1)
        ...
        plt.imshow(i_w_occ, vmax=1)
        plt.text(text_xy[0], text_xy[1], "Final synthesis (yellow: dis/occlusion)", fontdict=text_fd, size=10)
        plt.xticks([]) and plt.yticks([]) if args.save_fig else plt.grid(linestyle=":", linewidth=0.4)
        plt.xlim(0, 832 - 1)
        plt.ylim(256 - 1, 0)
        fig.add_subplot(gs[4, 0:2])
        plt.imshow(tgt_diff, cmap="bone", vmax=0.5)
        plt.text(text_xy[0], text_xy[1], "$I_{diff}$", fontdict=text_fd)
        plt.xticks([]) and plt.yticks([]) if args.save_fig else plt.grid(linestyle=":", linewidth=0.4)
        plt.xlim(0, 832 - 1)
        plt.ylim(256 - 1, 0)
        ...
        ax1.set_xlabel("X", fontdict=ax1_axfont)
        ax1.set_zlabel("Z", fontdict=ax1_axfont)
        ...
        ax1.text(
            ...
            0,
            zlim_1 * 1.20,
            "[Top-view] Objects in {$t+1$} frame on camera coordinate\n(red: ego-warped $t$→$t+1$, yellow: final-warped $t$→$t+1$, blue: $t+1$)",
            fontdict=ax1_titlefont,
        )
        if num_inst > 0:
            ax1.text(
                -xlim_1 / 2,
                0,
                zlim_1 * 1.10,
                "1: XYZ {}".format(r2t_obj_3d_locs[0].detach().cpu().numpy().round(4)),
                fontdict=ax1_annotfont,
                bbox=bbox_c,
            )
        if num_inst > 0:
            ax1.text(
                ...
                0,
                zlim_1 * 1.05,
                "1: XYZ {}".format(rtt_obj_3d_locs[0].detach().cpu().numpy().round(4)),
                fontdict=ax1_annotfont,
                bbox=bbox_y,
            )
        if num_inst > 0:
            ax1.text(
                ...
                0,
                zlim_1 * 1.00,
                "1: XYZ {}".format(tgt_obj_3d_locs[0].detach().cpu().numpy().round(4)),
                fontdict=ax1_annotfont,
                bbox=bbox_b,
            )
        if num_inst > 1:
            ax1.text(
                ...
                0,
                zlim_1 * 1.10,
                "2: XYZ {}".format(r2t_obj_3d_locs[1].detach().cpu().numpy().round(4)),
                fontdict=ax1_annotfont,
                bbox=bbox_c,
            )
        if num_inst > 1:
            ax1.text(
                ...
                0,
                zlim_1 * 1.05,
                "2: XYZ {}".format(rtt_obj_3d_locs[1].detach().cpu().numpy().round(4)),
                fontdict=ax1_annotfont,
                bbox=bbox_y,
            )
        if num_inst > 1:
            ax1.text(
                ...
                0,
                zlim_1 * 1.00,
                "2: XYZ {}".format(tgt_obj_3d_locs[1].detach().cpu().numpy().round(4)),
                fontdict=ax1_annotfont,
                bbox=bbox_b,
            )
        ...
        for ii in range(len(egoOs)):
            if ii >= len(egoOs) - 1:
                ...
                ax2.text(
                    ...
                    egoOs[ii, 1],
                    egoOs[ii, 2],
                    "{:0.4f}".format(np.linalg.norm(egoOs[ii] - egoOs[ii - 1]).round(4)),
                    fontsize=8,
                    bbox=bbox_w,
                    zorder=vidx + 5,
                )
            ...
            if ii >= len(objOs) - len(obj_pose):
                if np.linalg.norm(objOs[ii] - objHs[ii], 2) < 0.1 and np.linalg.norm(objOs[ii] - objHs[ii], 2) > 0.002:
                    dR.drawVector(
                        ...
                        pointEnable=False,
                        lineWidth=0.5,
                        faceColor=colors[objIDs_flatten[ii]],
                        edgeColor="k",
                        zorder=vidx + 20,
                    )
                    ax2.text(
                        ...
                        objHs[ii][1],
                        objHs[ii][2],
                        "{:0.4f}".format((np.linalg.norm(objOs[ii] - objHs[ii]) / obj_vo_scale).round(4)),
                        fontsize=8,
                        bbox=bbox_w,
                        zorder=vidx + 30,
                    )
        ...
        ax2.set_xlabel("X", fontdict=ax2_axfont)
        ax2.set_zlabel("Z", fontdict=ax2_axfont)
